...
def _test_match_redirect(
    map_: Map, path: str, method: str, redirect_path: str, query_string: bytes = b"", host: str = ""
) -> None:
...
def test_path_converter() -> None:
    ...
    _test_match(
        map_,
        "/branch/middle/leaf/constant",
        "GET",
        (map_.endpoints["double_page_constant"][0], {"left": "branch", "right": "leaf"}),
    )
    _test_match(map_, "/Colon:branch", "GET", (map_.endpoints["colon_base"][0], {"name": "branch"}))
    _test_match(
        map_,
        "/Colon:branch/leaf",
        "GET",
